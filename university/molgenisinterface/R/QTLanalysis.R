...
generateQTLfile <- function(DBmarkerID = 0, DBtraitID = 0, DBpath = "", job, b_size,Ntraits,name,taskID,map,method,model,step){
...
generateESTfile <- function(DBmarkerID = 0, DBtraitID = 0, DBpath = "",map,method,model,step,taskID){
...
est_runtime <- function(njobs,DBmarkerID=0, DBtraitID = 0,ntraits=1, DBpath = "",num_per_run=1,taskID,map,method,model,step){
	...
	generateESTfile(DBmarkerID, DBtraitID, DBpath,map,method,model,step,taskID)
	#time execution of executing 1 trait
	s <- proc.time()
	system("R CMD BATCH ESTtime.R")
	e <- proc.time()
	#Add 5% for security
	EST <- (num_per_run)*((e[3]-s[3])+20) 
...
report <- function(DBpath,task,job,status,text){
	#cat("report called for job:",job," with code",status,"\n")
	text <- substr(URLencode(text),0,100)
	link <- paste(DBpath,"/taskreporter?task=",task,"&job=",job,"&statuscode=",status,"&statustext=",text,sep="")
	getURL(link)
	if(status==-1){
		#cat("!!!",textm,"!!!")
		q("no")
...
run_cluster <- function(name = "qtlTEST",genotypes = 0, phenotypes = 0,totalitems=1, njobs=1,dbpath = "",taskid=1,map="scanall",method="hk",model="normal",step=0){
	...
	tryCatch(system("R CMD BATCH download.R")
		,error =  function(e){report(dbpath,taskid,0,-1,"DownloadingCrossobject")}
	)
	report(dbpath,taskid,0,2,"FinishedDownloadingDatasets")	
	tryCatch(est <- est_runtime(njobs,genotypes,phenotypes,totalitems,DBpath=dbpath,nprun,taskid,map,method,model,step)
	...
	for(x in 1:njobs){
		...
		tryCatch(generateQTLfile(DBmarkerID=genotypes, DBtraitID=phenotypes, DBpath=dbpath, job=x, b_size=nprun,Ntraits=totalitems,name=name,taskid,map,method,model,step)
		...
		tryCatch(system(paste("qsub ",runfile,sep=""))
			,error =  function(e){report(dbpath,taskid,x,-1,"SubmissionPBS:")}
		)
