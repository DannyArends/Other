...
goes wrong. We can use the numeric class to denote a sequence of numbers but we can also create 
...
\end{verbatim}
{\bf Assignments}\\
  {\bf A0.0} How do we check if something is between 0 and 10, (and error otherwise)\\
  {\bf A0.1} Use a for and a while loop from 1 to 1000 and add up all the numbers in a new variable
  {\bf A0.2} Use cat and paste to print out a triangle of \#
\begin{verbatim}
1 #
2 ##
3 ###
...
\begin{verbatim} read.table("dataset1.txt") \end{verbatim} 
  This will print out the contents of the datafile to the screen if successful. NORMALLY this is 
  not the case. Additional parameters will have to be provided, so that R can make sense of data 
  format. Parameters like {\bf header}, {\bf row.names}, {\bf sep}, {\bf fill}. It it wise to first 
  open the data file in a text editor and check if there are any headers, row names, empty lines, 
