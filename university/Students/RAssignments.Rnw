...
too low, the heater should be ON. IF the temperature is too high the cooler needs to be ON and the heater OFF. Basic decisions like this 
can be used to build complex behaviors. This control flow plus the ability to store results in variables allow us to manipulate data, store 
...
An example of using an IF statement can look something like this:
\begin{verbatim}
...
Furthermore, it is a common practice to exercise a kind of ownership. So people know who wrote the code and e.g. who is 
currently maintaining it. This might seem like a waste of time now, and it is. But when working with more 
people on larger projects it is impossible to keep track of everything unless the code is well documented.
\\\\
{\bf Assignments}
\\
...
\subsection{Loading data sets}
  Loading data sets is done with the {\bf read.table} and {\bf read.csv} functions. They are both similar in
  the options they provide for loading data. But load.csv is generally faster by a factor of two to four in 
  loading large data sets. An example to load filename.txt into the R environment:
...
\begin{verbatim} setwd("c:/users/danny/files/data") #Note the \ is turned into a / \end{verbatim} 
  This will change the current working directory. many functions have their counterpart and 
  {\bf setwd()} has {\bf getwd()} to get the current directory. 

  After setting the correct work directory you can load a datafile into R by typing:
\begin{verbatim} read.table("dataset1.txt") \end{verbatim} 
  This will print out all the content of the data file on to the screen, and stop when it encounters an 
  error. Normally you do not want to have 15 gigabyte of data rolling before your eyes (takes around 
  two to four minutes on a slow PC). Even worse, in many cases you will just be presented with an error 
  such as: 'Line XX does not contain XX elements'. Errors in this case mean data is not formatted 
  correctly, which can normally be solved by informing R about the format of your data. To do this the 
  {\bf read.table} and {\bf read.csv} functions provide additional parameters so that R can make sense 
  of data format. Take some time to read through the parameter descriptions in the help files from 
  these two functions, because you will be using them a LOT.
  ...
  It it a normally a good idea to first open the data file in a text editor and check if there are any headers, 
  ...
  a box where you can store values in. To assign the output of the read.table function, we can use the 
  <- assignment operator.
...
many ways to statistically test for e.g. Normality of your data, but a quick look by eye will usually be just as informative.
...
r1 <- runif(100)*5
r2 <- runif(100)*5
t.test(r1,r2) #Above 0.05 so we accept the 0-hypothesis
@
  {\bf Assignments}\\
...
\section{R and the WWW}
  We can interact with web pages in an easy way using the Rcurl package.
...
\subsection{RCurl}
  ...
  \\
  {\bf Assignments}\\
  {\bf A8.0} Install the Rcurl package and load http://www.google.com using the getURL function\\
  {\bf A8.1} Create a function to retrieve the Google homepage image and save it to the hard drive\\
  {\bf A8.2} Create a function that posts a searchterm to Google and saves the resulting HTML file to disk\\
  {\bf A8.3} Screenscape the return of A8.2 and parse out the hyperlinks google returns\\

\subsection{Databases}
  To come soon..

\subsection{Show your work online}
  To come soon..
...
R packages are defined in a rigid structure to harmonize across all different R versions and platforms. 
Additionally the structure provides a nice self contained unit of code and documentation, which can be 
easily ported/updated when a new version of R comes out. This allows R developers to maintain R and Rgui, 
while the community maintains the different packages. When R versions are updated, developers are 
responsible themselves for creating an updated version of their package for the new version.
\begin{itemize}
...
\end{itemize}
For more on creating packages and how to submit them to CRAN just use a search engine, there are many 
resources out there on best practices when building an R package.
\newpage
\section{R cheat sheet}
The cheat sheet is available at:\\
\url{http://www.dannyarends.nl/pdf/Rcheats.pdf}
