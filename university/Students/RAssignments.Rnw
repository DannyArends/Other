...
$^{2}$Faculty of Biochemistry, Biophysics and Biotechnology, Jagiellonian University in Kraków, PL.\\
}
...
Notepad++ is a text editor for editing text-documents and opening larger data sets it is open-source and has many 
additional features, like syntax highlighting for several programming languages. Other IDE's can of course be used 
as well.
\end{itemize}
Programming is more or less learned by doing, and trying and failing and frustration which are compensated by the eureka moments.
...
languages use data types to tell the CPU how many rows lights to use for a certain data type.
...
@
To create an object of a string class, we can concatenate it together with a String value. String values are 
surrounded by quotes ", also when R print them to the screen. If we concatenate a number and a string, the result 
will be a single string with the string concatenated to the numeric value. This can be used to create e.g. names 
for our objects/individuals. For this we have two functions {\bf cat} and {\bf paste}. When we supply a sequence 
of values/strings {\bf cat} creates one big string while {\bf paste} will create a vector of strings. An example:
...
Controlling a program is done by executing an algorithm, best to be thought of as a cook receipt. The computer diligently follows 
...
\end{itemize}
Control structures shape the flow in a program. Based on decision points we can take 
...
\begin{verbatim} invisible(result_variable) \end{verbatim}
  This is extremely useful when dealing with BIG data sets. Printing of 
...
\section{Displaying your data}
  ...
  The device in our case is usually a image type: like PNG, JPG, GIF, EPS. For bio informatics 
  ...
  routines. To open a device you can simply call its name and add parameters to control things like 
...
\end{itemize}
The descriptive statistics can be useful to get hands on with data (e.g. detect outliers, find batch effects). Also plotting the 
correlation between individual samples can give an indication of the quality of the whole data set.
However R also contains a scala of modeling tools like: Fitting and Testing models to data
Functions that model data take a model function description, so lets first look at an example. We create $y$ holding a linear set of 
response variables, we add a linear effect of 0.5 and la small amount of noise (y1) and do the same again only a smaller effect and 
