...
Remember these functions, they are among the most usefull functions that R provides.
These functions can be used to make subsets of your data at minimal effort. So take 
a moment to familiarize yourself with these functions by tooking up their help files 
...
\section{Displaying your data}
  ...
  Visual output (plots / images) can be saved by specifing an output file with a {\bf device}. 
  The device in our case is usualy a image type: like PNG, JPG, GIF, EPS. For bioinformatics 
