/**
 * \file MYMATH/geneticalgorithm.cpp
 * \brief Code file, Implementation of concept: geneticalgorithm
 *
 * Implementation of a genetic algorithm  to reach a floatvalue in a maximum number of itteration
...
bytechromosome::bytechromosome(){
  this->bits ="";
  this->fitness = 0.0f;
}

bytechromosome::bytechromosome(string bts, float ftns){
  this->bits = bts;
  this->fitness = ftns;
}

string bytechromosome::getbits(){
  return this->bits;
}

float bytechromosome::getfitness(){
  return this->fitness;
}

void bytechromosome::setbits(string bts){
  this->bits=bts;
}

void bytechromosome::setfitness(float ftns){
  this->fitness=ftns;
}
